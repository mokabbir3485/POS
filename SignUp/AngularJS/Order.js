...
orderApp.controller('orderController', ['$scope', '$filter', '$http', '$window', 'orderService', function ($scope, $http, $filter, $window, orderService) {
    ...
    $scope.Delete = function (OrderObj) {
        orderService.GetAllOrderDetail(OrderObj).then(function (responset) {
            $scope.AddproductList = responset.data;

            orderService.GetAllProduct().then(function (responsep) {
                ...
                if ($scope.AddproductList.length > 0 && $scope.productList.length > 0) {
                    ...
                    if (r == true) {
                        orderService.PostProduct($scope.AddproductList).then(function (response) {

                        });
                        orderService.DeleteOrder(OrderObj).then(function (response) {
                            alert("OrderObj Deleted Successfully");
            ...
            alert("Failed to get OrderObj Detail!");
        })
